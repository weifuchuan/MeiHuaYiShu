...
import { Card, Button, List, DatePicker, Picker } from 'antd-mobile-rn';
...
@inject('store')
@observer
class HouTianGua extends React.Component<
...
> {
		...
		return (
			<View style={styles.container}>
				<ListRow
					title={'上卦：'}
					detail={
						<Text style={{ color: 'blue' }} onPress={() => LeiXiang.modalShow()}>
							万物类象
						</Text>
					}
					topSeparator={'full'}
					...
					accessory={
						<Select
							...
							getItemText={(item: string) =>
								`${item}(${gua.directions[gua.words.findIndex((w) => w === item)!]})`}
							value={this.bottom}
							pickerType={'popover'}
							onSelected={(item: gua.GuaW) => {
								this.bottom = item;
	...
	private qiGua = () => {
		const qg = qiGuaByHouTian(
			gua.words.lastIndexOf(this.up) as gua.GuaNumber,
