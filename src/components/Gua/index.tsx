...
@inject('store')
@observer
export default class Gua extends React.Component<
						...
						<TabView
							...
							renderTabBar={(props) => (
								<TabBar
									...
									renderLabel={(params) => (
										<Text style={{ color: '#000' }}>{(params.route as any).title}</Text>
									)}
								/>
							)}
						/>
...
const YangYao = ({ color, style = {} }: { color: string; style?: ViewStyle }) => {
	return <View style={Object.assign(style, { backgroundColor: color, width: yaoWidth, height: yaoHeight })} />;
};
...
const YiGua = ({
	...
	const arr = [
		{ what: g[0], color: colors[0] },
		null,
		{ what: g[1], color: colors[1] },
		null,
		{ what: g[2], color: colors[2] }
	];
	return (
		<View
			style={Object.assign(style, {
				width: yaoWidth,
				height: yaoHeight * 3 + yaoHeight * 0.6 * 2,
...
export const ZhenGua = ({ qg, style = {} }: { qg: gua.QuanGua; style?: ViewStyle }) => {
	...
	return (
		<View
			style={Object.assign(style, { width: width * 0.9, flexDirection: 'row', justifyContent: 'space-around' })}
		>
			<View style={{ flexDirection: 'column-reverse' }} key={-1}>
				<YiGua
					color={relation2color.get(getRelationForGua(qg.gua[0], t))!}
					g={qg.gua[0]}
					by={qg.ti === 1 ? qg.bian as 1 | 2 | 3 : 0}
				/>
				<LargeRowSeparator />
				<YiGua
					color={relation2color.get(getRelationForGua(qg.gua[1], t))!}
					g={qg.gua[1]}
					by={qg.ti === 0 ? (qg.bian - 3) as 1 | 2 | 3 : 0}
				/>
			</View>
	...
	return (
		<ScrollView style={{ flex: 1 }}>
			<View style={{ padding: width * 0.05 }}>
				<Text selectable={true} style={{ marginBottom: width * 0.02 }}>{`${g.name}：${g.name2}，上${g.pailie
					.up}下${g.pailie.bottom}。`}</Text>
				<Text selectable={true} style={{ marginBottom: width * 0.02 }}>
					{g.profile}
				</Text>
				{g.yaos.map((yao, i) => (
					<Text selectable={true} key={i} style={{ marginBottom: width * 0.02 }}>{`${yao[0]}：${yao[1][
						yao[1].length - 1
					] === '。'
						? yao[1]
						: yao[1] + '。'}`}</Text>
