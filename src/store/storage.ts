...
export default class Storage {
  ...
  constructor(options: any = {}) {
    ...
    this._mapPromise = this.getItem('map').then((map: any) => {
  ...
  _getId(key: any, id: any) {
    return key + '_' + id;
  }
  _saveToMap(params: any) {
  ...
  getBatchDataWithIds(params: any) {
    let { key, ids, syncInBackground } = params;

    return Promise.all(
      ids.map((id: any) => this.load({ key, id, syncInBackground, autoSync: false, batched: true }))
    ).then((results) => {
      return new Promise((resolve, reject) => {
        const ids = results.filter((value: any) => value.syncId !== undefined);
        if (!ids.length) {
          return resolve();
        }
        return this.sync[key]({
          id: ids.map((value: any) => value.syncId),
          resolve,
          reject
        });
      }).then((data: any) => {
  ...
  _lookupGlobalItem(params: any) {
    ...
    return this.getItem(key).then((ret: any) => this._loadGlobalItem({ ret, ...params }));
  }
  _loadGlobalItem(params: any) {
  ...
  _lookUpInMap(params: any) {
    ...
    if (m[newId] !== undefined) {
      return this.getItem('map_' + m[newId]).then((ret: any) => this._loadMapItem({ ret, ...params }));
    }
